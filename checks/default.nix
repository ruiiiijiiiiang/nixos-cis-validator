...
nixpkgs.lib.genAttrs supportedSystems (
  system: let
    ...
    mkSystemWith = hostConfig: validatorConfig:
      lib.nixosSystem {
        inherit system;
        modules = [
          (import ../modules)
    ...
    mkSystem = mkSystemWith {};
    mkOpenSshSystem = mkSystemWith {services.openssh.enable = true;};
    ...
    acceptedException = mkOpenSshSystem {
      enable = true;
      failureMode = "error";
      defaultRuleEnable = false;
      rules."5.1.20" = {
    ...
    selectiveBlocking = mkOpenSshSystem {
      enable = true;
      failureMode = "report";
      defaultRuleEnable = false;
      rules."5.1.20" = {
  ...
  in {
    ...
    report-only =
      pkgs.runCommand "cis-validator-report-only-check" {
        ...
        jq -e '
          ...
          (.rules | map(select(.source.recommendation == "5.1.20"))[0].title) == "Ensure sshd PermitRootLogin is disabled" and
    ...
    openssh-disabled = assert !lib.any (message: lib.hasInfix "CIS 5.1.10" message) openSshDisabled.config.warnings;
      pkgs.runCommand "cis-validator-openssh-disabled-check" {
        nativeBuildInputs = [pkgs.jq];
        report = openSshDisabled.config.system.build.cisValidationReport;
      } ''
        jq -e '
          .summary.enabledRules == 1 and
          .summary.violations == 0 and
          .summary.notApplicable == 1 and
          (.rules | map(select(.source.recommendation == "5.1.10"))[0].status) == "not-applicable" and
          (.rules | map(select(.source.recommendation == "5.1.10"))[0].reason) == "The NixOS OpenSSH server is disabled."
        ' "$report" >/dev/null
        touch "$out"
      '';
  }
